...
function EditForm({ data, addStatus }) {
  ...
  useEffect(() => {
    if (addStatus) {
      console.log('edit', openStatus)
      let postData = {
        open: openStatus,
        ...inputs
      }
      console.log('edit', postData)
      axios.put(`/api/inquiry/${data.data.id}`, postData).then(v => {
        history.push('/')
      })
    } else {
    ...
    <>
      <div className="flex flex-wrap mt-7 mx-7" style={{ minWidth: '1000px' }}>
        ...
        <ul className="flex-1 flex flex-col border-2 border-yellow-500 rounded-md text-center">
          ...
          <li>
            <input className="p-2 w-full outline-none rounded-b-md" name="password" onChange={onChange} value={password} />
          </li>
        </ul>
      </div>
    </>
